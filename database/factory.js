...
Factory.blueprint('App/Models/Cinema', async faker => {
  const cinemaName = faker.company()
  return {
    cinema_name: cinemaName,
    cinema_screenshot: `http://fakeimg.pl/600x400/?text=${cinemaName}`,
...
Factory.blueprint('App/Models/Movie', async faker => {
  const movieName = faker.sentence({ words: 3 })
  return {
    movie_name: movieName,
    movie_director: faker.name(),
    movie_screenshot: `http://fakeimg.pl/600x400/?text=${movieName}`,
    movie_sinopsis: faker.sentence({ words: 50 }),
    created_at: faker.date()
  }
})
...
Factory.blueprint('App/Models/Seat', async (faker, i, data) => {
  return {
    ...
    seat_row: faker.integer({ min: 1, max: 100 }),
    seat_number: faker.integer({ min: 1, max: 10 }),
    seat_state: 'BOOKED'
  }
})
